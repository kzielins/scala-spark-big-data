...
class ListsSuite extends munit.FunSuite:
  ...
  test("max(List()) throws an exception ") {
    try
      max(List())
      fail("max List(Nil)")
    catch
  ...
  test("sum of 4  numbers ===10 ") {
    assertEquals(sum(List(1, 2, 3, 4, 0)), 10)
  }

  test("max of 4 numbers ==10 ") {
    assertEquals(max(List(1, 2, 10, 3, 4)), 10)
  }
